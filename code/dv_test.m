clear all
clc
path = '..\..\results_efdv-jcn\';
read_I = dir([strcat(path, 'originales\ROI_1656-6756-329\'), '*.png']);
read_nucprop = dir([strcat(path, '01_nucleis\'), '*.png']);
read_cell = dir([strcat(path, '08_resCell\'), '*.png']);

for k=1:length(read_I)
    fprintf('%i\n', k);
    files_I = read_I(k).name;
    files_propN = read_nucprop(k).name;
    I = imread(strcat(path, 'originales\ROI_1656-6756-329\',files_I));
    nucprop = imread(strcat(path, '01_nucleis\',files_propN));
    cell = imread(strcat(path, '08_resCell\',files_propN));
...
%                     tamobj2 = numNucs(n);
    ...
    imwrite(nucleos, strcat(path,'09_nucs2\',files_I));
end
